...
class Counter(dict):
    ...
    def arg_max(self):
        ...
        all_idxs = list(self.items())
        values = [x[1] for x in all_idxs]
        max_index = values.index(max(values))
        return all_idxs[max_index][0]
    ...
    def sorted_keys(self):
        ...
        sorted_items.sort(key=cmp_to_key(compare))
...
def normalize(vector_or_counter):
    ...
    if vector_or_counter is normalized_counter:
...
def choose_from_distribution(distribution):
    ...
    if (distribution is dict) or (distribution is Counter):
